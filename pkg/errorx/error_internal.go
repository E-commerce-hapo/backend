...
func newInternalError(code int, message string, err error) *internalError {
	if message == "" {
...
func (t *internalError) WithMetaM(m map[string]string) ErrorInterface {
	if t.Meta == nil {
		t.Meta = m
	} else {
		for k, v := range m {
...
func ToErrorInterface(err error) ErrorInterface {
	...
	if !ok {
		xerr = newInternalError(http.StatusInternalServerError, "", err)
	}
